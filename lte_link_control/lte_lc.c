...
#if 0 //jcs
static const char unsubscribe[] = "AT+CEREG=0";
#endif
...
#endif
#if 1 // jcs: to allow runtime selection of network mode before lte_lc_init_and_connect()
static int8_t network_mode_index = -1;
static const char network_mode_override[2][23] = {{"AT%XSYSTEMMODE=1,0,0,0"}, {"AT%XSYSTEMMODE=0,1,0,0"}};
#endif
...
#endif
#if 1 // jcs: to allow runtime selection of network mode before lte_lc_init_and_connect()
if (network_mode_index != -1)
{
	if (at_cmd_write(network_mode_override[network_mode_index], NULL, 0, NULL) != 0) {
		return -EIO;
	}
}
else
...
#if 0 //jcs
	if (at_cmd_write(unsubscribe, NULL, 0, NULL) != 0) {
		return -EIO;
	}
#endif
...
#if 1 // jcs: to allow runtime selection of network mode before lte_lc_init_and_connect()
void lte_lc_set_network_mode_to_LTE_M()
{
    network_mode_index = 0;
}

void lte_lc_set_network_mode_to_NB_IoT()
{
    network_mode_index = 1;
}
#endif
